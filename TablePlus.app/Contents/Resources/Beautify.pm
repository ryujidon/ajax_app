...
sub new {
    ...
    for my $key ( qw( query spaces space indent break wrap keywords functions rules uc_keywords uc_functions no_comments placeholder separator comma comma_break format colorize format_type wrap_limit wrap_after) ) {
...
sub tokenize_sql {
    ...
    my $re = qr{
        (
                ...
                |
                x'(?>(?:(?>[^'\\]+)|''|\\.)*)+' # anything escaped inside single quotes, ungreedy.
                |
                X'(?>(?:(?>[^'\\]+)|''|\\.)*)+' # anything escaped inside single quotes, ungreedy.
                |
...
sub set_dicts {
    ...
    my @pg_keywords = map { uc } qw( 
        ...
        LOOP MAPPING MAXVALUE MINVALUE MODULUS MOVE NATURAL NEXT OFFSET
    ...
    my @pg_functions = map { lc } qw(
        adddate addtime aes_decrypt aes_encrypt and any_value asymmetric_decrypt asymmetric_derive asymmetric_encrypt asymmetric_sign
        asymmetric_verify benchmark between bin bin_to_uuid binary bit_count bit_xor can_access_column can_access_database can_access_table
        can_access_view case charset coercibility collation compress connection_id conv convert_tz crc32 create_asymmetric_priv_key
        create_asymmetric_pub_key create_dh_parameters create_digest curdate current_role curtime database date_add date_format date_sub datediff day
        dayname dayofmonth dayofweek dayofyear default des_decrypt des_encrypt elt export_set extractvalue field find_in_set format_bytes
        format_pico_time found_rows from_base64 from_days from_unixtime geomcollection geometrycollection get_dd_column_privileges
        get_dd_create_options get_dd_index_sub_part_length get_format get_lock greatest group_concat grouping gtid_subset gtid_subtract hex hour
        icu_version if ifnull in inet_aton inet_ntoa inet6_aton inet6_ntoa insert instr internal_auto_increment internal_avg_row_length
        internal_check_time internal_checksum internal_data_free internal_data_length internal_dd_char_length internal_get_comment_or_error
        internal_get_view_warning_or_error internal_index_column_cardinality internal_index_length internal_keys_disabled internal_max_data_length
        internal_table_rows internal_update_time is is_free_lock is_ipv4 is_ipv4_compat is_ipv4_mapped is_ipv6 is_used_lock is_uuid isnull json_array
        json_array_append json_array_insert json_arrayagg json_contains json_contains_path json_depth json_extract json_insert json_keys
        json_length json_mergejson_merge_preserve json_merge_patch json_merge_preserve json_objectagg json_pretty json_quote json_remove json_replace
        json_search json_set json_storage_free json_storage_size json_table json_type json_unquote json_valid last_day last_insert_id lcase least
        left like linestring load_file locate log10 log2 make_set makedate maketime master_pos_wait match mbrcontains mbrcoveredby mbrcovers
        mbrdisjoint mbrequals mbrintersects mbroverlaps mbrtouches mbrwithin microsecond mid minute month monthname multilinestring multipoint
        multipolygon name_const not nullif oct or ord password period_add period_diff ps_current_thread_id ps_thread_id quarter quote rand
        random_bytes regexp regexp_instr regexp_like regexp_substr release_all_locks release_lock right rlike roles_graphml row_count schema
        sec_to_time second sha1 sha2 sleep soundex sounds space st_area st_asbinary st_asgeojson st_astext st_buffer st_buffer_strategy st_centroid
        st_contains st_convexhull st_crosses st_difference st_dimension st_disjoint st_distance st_distance_sphere st_endpoint st_envelope
        st_equals st_exteriorring st_geohash st_geomcollfromtext st_geomcollfromwkb st_geometryn st_geometrytype st_geomfromgeojson st_geomfromtext
        st_geomfromwkb st_interiorringn st_intersection st_intersects st_isclosed st_isempty st_issimple st_isvalid st_latfromgeohash st_latitude
        st_length st_linefromtext st_linefromwkb st_longfromgeohash st_longitude st_makeenvelope st_mlinefromtext st_mlinefromwkb st_mpointfromtext
        st_mpointfromwkb st_mpolyfromtext st_mpolyfromwkb st_numgeometries st_numinteriorring st_numpoints st_overlaps st_pointfromgeohash
        st_pointfromtext st_pointfromwkb st_pointn st_polyfromtext st_polyfromwkb st_simplify st_srid st_startpoint st_swapxy st_symdifference
        st_touches st_transform st_union st_validate st_within st_x st_y statement_digest statement_digest_text std str_to_date strcmp subdate
        substring_index subtime sysdate system_user time_format time_to_sec timediff timestampof the arguments timestampadd timestampdiff to_base64
        to_days to_seconds truncate ucase uncompress uncompressed_length unhex unix_timestamp updatexml user utc_date utc_time utc_timestamp uuid
        uuid_short uuid_to_bin validate_password_strength values wait_for_executed_gtid_set wait_until_sql_thread_after_gtids week weekday
